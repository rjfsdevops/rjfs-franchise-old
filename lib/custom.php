...
function rjfs_post_type_lists()
{
    ...
    $args = array(
        ...
        'description' => __('Make lists to use throughout the site', 'text_domain'),
        'labels' => $labels,
        ...
        'publicly_queryable' => true,
//        'capability_type' => 'page',
        'register_meta_box_cb' => 'rjfs_list_icon_meta_box_add',
    );
    register_post_type('list', $args);

    //register lists collections
    $labels = array(
...
// Hook into the 'init' action
add_action('init', 'rjfs_post_type_lists', 0);

//create some meta boxes
function rjfs_list_icon_meta_box()
{
    global $post;

    // Noncename needed to verify where the data originated
    echo '<input type="hidden" name="listmeta_noncename" id="listmeta_noncename" value="' .
        wp_create_nonce(plugin_basename(__FILE__)) . '" />';

    $types = array(
        'Bootstrap Glyphicons' => 'glyphicons',
        'Font Awesome 4.0' => 'fontawesome',
    );
    // Get the location data if its already been entered
    $type = get_post_meta($post->ID, 'list_icon_type', true);
    $selected = get_post_meta($post->ID, 'list_type_icon', true);

    global $icons;
    echo '<script>';
    echo ' RJFS.glyphicons = ' . json_encode($icons['glyphicons']) . ';';
    echo ' RJFS.fontawesome = ' . json_encode($icons['fontawesome']) . ';';
    echo '</script>';

    // Echo out the field
    echo '<p class="meta-options">';
    foreach ($types as $key => $val) {
        echo '<label for="list_icon_type_' . $val . '" class="selectit"><input name="list_icon_type" type="radio" id="list_icon_type_' . $val . '" value="' . $val . '" ' . (($val == $type) ? 'checked' : '') . '>' . $key . '</label>';
        echo '<br/>';
    }
    echo '</p>';

    echo '<p class="meta-options" ><label for="list_type_icon">Icon:<br><select name="list_type_icon" id="list_type_icon">';
    if (!empty($type)) {
        //list out some options
        foreach ($types as $key => $value) {

            if ($value == $type) {
                foreach ($icons[$value] as $icon => $val) {
                    echo '<option value="' . $val . '" ' . (($val == $selected) ? 'selected' : '') . '>' . ((!empty($val)) ? $val : $icon) . '</option>';
                }
            }
        }
    }
...
//add some metabox to lists
function rjfs_list_icon_meta_box_add()
{
    add_meta_box('rjfs-list-icon', 'List Icon', 'rjfs_list_icon_meta_box', 'list', 'normal', 'high');
}


// Save the Metabox Data

function rjfs_list_icon_meta_box_save($post_id, $post)
{

    // verify this came from the our screen and with proper authorization,
    // because save_post can be triggered at other times
    if (!wp_verify_nonce($_POST['listmeta_noncename'], plugin_basename(__FILE__))) {
        return $post->ID;
    }
    ...
    $events_meta['list_icon_type'] = $_POST['list_icon_type'];
    $events_meta['list_type_icon'] = $_POST['list_type_icon'];
...
add_action('save_post', 'rjfs_list_icon_meta_box_save', 1, 2); // save the custom fields

// create shortcode to list all clothes which come in blue
function rjfs_list_icon_shortcode($atts)
{
    ob_start();
...
add_shortcode('RJFS-List', 'rjfs_list_icon_shortcode');
